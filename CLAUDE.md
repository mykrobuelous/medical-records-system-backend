...
Backend API for a medical records system (patients, consultations, and lookup resources for insurance/diagnosis/medicine), with JWT-based authentication gating all resource routes. Express 5 + TypeScript, running as native ESM, with Drizzle ORM over PostgreSQL. All five resources — `patients`, `consultations`, `insurances`, `diagnoses`, `medicines` — have full CRUD routes/controllers; there is no frontend in this repo.
...
Seed the database via the running API (not a script): `POST /api/seed` (unauthenticated — mounted before `authenticateUser` in `index.ts`) — deletes and re-inserts all patients/consultations/insurances/diagnoses/medicines from `src/data/seedData.ts` inside a single transaction (`seedRouter.ts`).
...
- **Lookup resources** (`insurances`, `diagnoses`, `medicines` in `schema.ts`): simple standalone tables (no relations, no FK to patients/consultations) each with an `id` plus one or two descriptive fields (`insurance`, `diagnosis`, or `medicine`+`description`). `consultations.insurance` stores either an `insurances.id` or the literal string `'Personal'` (self-pay) as a plain `varchar`, not a DB-level FK — that's deliberate since `'Personal'` isn't a real `insurances` row, so validity is enforced at the zod layer, not by Postgres. Their routes/controllers/schemas follow the same three-file split as `patients`/`consultations` but are simpler CRUD with no cascade or FK-violation handling to worry about.
- **Types vs schema duplication**: `src/data/data.types.ts` defines `PatientType`/`ConsultationType`/`InsuranceType`/`DiagnosisType`/`MedicineType` independently from the Drizzle schema (not inferred via `typeof patients.$inferSelect`). These are used by the hand-written seed data in `src/data/seedData.ts`. When changing a table shape, update the Drizzle schema, these hand-rolled types, and the corresponding zod schema in the resource's `.schema.ts` together.
